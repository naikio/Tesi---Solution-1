...
int AcquireBodyFrameAndJoints(AcquisitionKinect2& acq, IBodyFrameReader* bodyReader, ICoordinateMapper* coordinateMapper, Point3d bodySpacePoints[BODY_COUNT]){
	// BODY reader
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	while (!GetAsyncKeyState(VK_ESCAPE)){
		...
		//will store NECK POINTS for each tracked body --GROUND TRUTH DATA--
		Point3d bodySpacePoints[BODY_COUNT];
		...
		if (AcquireBodyFrameAndJoints(acq, bodyReader, coordinateMapper, bodySpacePoints)!= 0){
			continue;
		}
